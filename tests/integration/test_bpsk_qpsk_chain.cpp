...
TEST_F(QpskChainTest, PerfectChannelYieldsZeroBer_QPSK) {
...
TEST_F(QpskChainTest, BerDecreasesAsSnrIncreases_QPSK) {
...
TEST_F(QpskChainTest, BerAt10dBCoherentWithTheory_QPSK) {
    // BER theorique QPSK (le meme pour BPSK) a 10 dB ~ 3.87e-6 => BER = Q(√(2 * Eb/N0))
...
TEST_F(QpskChainTest, BerAt0dBAroundExpectedValue_QPSK) {
...
TEST_F(QpskChainTest, Sent8BitsEqualReceivedBitsOnPerfectChannel_QPSK) {
    AwgnChannel channel(100.0, 42);
...
// ── Test 1 : canal parfait -> BER = 0 ────────────────────────────────────────
TEST_F(BpskChainTest, PerfectChannelYieldsZeroBer_BPSK) {
    // SNR = 100 dB ~ canal parfait
    AwgnChannel channel(100.0, 42);
...
// ── Test 2 : BER diminue quand SNR augmente ───────────────────────────────────
TEST_F(BpskChainTest, BerDecreasesAsSnrIncreases_BPSK) {
    const int num_bits = 50000;
    auto bits = generateRandomBits(num_bits, 42);
...
// ── Test 3 : BER a SNR=10dB coherent avec la theorie BPSK ────────────────────
TEST_F(BpskChainTest, BerAt10dBCoherentWithTheory_BPSK) {
...
// ── Test 4 : BER a SNR=0dB autour de 0.08 ────────────────────────────────────
TEST_F(BpskChainTest, BerAt0dBAroundExpectedValue_BPSK) {
    // BER theorique BPSK a 0 dB ~ 0.0786 => BER = Q(√(2 * Eb/N0))
    // On verifie que BER est dans [0.05, 0.15] — fourchette large
    // pour tenir compte de la variance statistique
    const int num_bits = 100000;
...
// ── Test 5 : 8 bits envoyes = 8 bits recus sur canal parfait ─────────────────────
TEST_F(BpskChainTest, Sent8BitsEqualReceivedBitsOnPerfectChannel_BPSK) {
    AwgnChannel channel(100.0, 42);
